...
class ModulatedConv2d(nn.Module):
    ...
    def get_style(self, style):
        batch = style.size(0)
        return self.modulation(style).view(batch, 1, -1, 1, 1)
    
    def apply_style(self, input, style):
    ...
    def forward(self, input, style):
        ...
        out = self.apply_style(input, style)
        return out
...
class ToRGB(nn.Module):
    ...
    def apply_style(self, input, style, skip=None):
        out = self.conv.apply_style(input, style)
        out = out + self.bias
    ...
    def forward(self, input, style, skip=None):
        style = self.get_style(style)
        out = self.apply_style(input, style, skip=skip)
        return out
...
class StyledConv(nn.Module):
    ...
    def get_style(self, style):
        style = self.conv.get_style(style)
        return style
    
    def apply_style(self, input, style, noise=None):
        out = self.conv.apply_style(input, style)
    ...
    def forward(self, input, style, noise=None):
        style = self.get_style(style)
        out = self.apply_style(input, style, noise=noise)
        return out
...
class Generator(nn.Module):
    ...
    def z_to_w(self, z, truncation=1.0, truncation_latent=None):
        w = self.get_latent(z)
        if truncation < 1:
            w = truncation_latent + truncation * (style - truncation_latent)
        w = w.unsqueeze(1)#.repeat(1, self.n_latent, 1)
        return w
    
    def w_to_ys(self, w):
        ys = []
        ys.append(self.conv1.get_style(w))
        ys.append(self.to_rgb1.get_style(w))
        for conv1, conv2, to_rgb in zip(self.convs[::2], self.convs[1::2], self.to_rgbs):
            ys.append(conv1.get_style(w))
            ys.append(conv2.get_style(w))
            ys.append(to_rgb.get_style(w))
        return ys
